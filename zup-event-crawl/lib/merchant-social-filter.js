...
const BLOCKED_CATEGORY = /快餐|小吃|便利|超市|购物|维修|修理|美容|美发|民宿|烤串|烧烤|麻辣烫|奶茶|咖啡(?!.*酒)|酒店(?!.*餐吧)|游玩|海鲜|美食城|面包蛋糕|馄饨|抄手|扁食|儿童乐园|舒适型|经济型|青年旅舍|豪跑车租赁|茶饮果汁|饮品|健身|瑜伽|普拉提|其他分类/i;

const BAR_NAME_SIGNAL = /Taproom|taproom|酒馆|酒肆|酒窖|精酿|啤酒屋|Brew|BREW|酒吧|清吧|小酒馆|Hookah|hookah|shisha|COMMUNE|Live\s*House|啤酒花园|Goose\s*Island|鲜酿|啤酒|跳海|京A|bar/i;
...
const JUNK_NAME_SIGNAL = /快餐|美甲|租车|回收|超市|便利|维修|修理|纱窗|门窗|批发|卖场|机床|摩托车租赁|街舞|刺青|公寓$|老酒回收|酒水$|取景地|美食城|游乐场|青年旅舍|旅舍|的虾|文和友|联名|云吞|馄饨|超跑|租赁|乒乓空间|^乒乓|歇业|停业|尚未开业|未开业|即将开业|瑜伽|普拉提|健身工作室|塑形|卡牌|桌游/i;
...
function matchesSocialVenueIntent(item, context = {}) {
  ...
  if (
    !context.skipKeywordCheck
    && !isBrandSearch
    && searchKeyword
    && !nameIncludesKeyword(name, searchKeyword)
  ) {
  ...
  if (BLOCKED_CATEGORY.test(category) && !looksLikeBar) {
