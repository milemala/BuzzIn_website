...
4. **[`docs/event-poi-agent-workflow.md`](docs/event-poi-agent-workflow.md)** — **POI 匹配**（搜词、decisions、常见翻车）

用户说「抓取某城豆瓣活动」时，**必须先读 3、4**，再执行 `.cursor/rules/douban-crawl-and-agent-poi.mdc`：**先分类入库，再 POI**。规则以仓库文档为准，不依赖聊天上下文。
