...
//
//  4. 气泡挂商户（二选一，详见 changeLog2.md）：
//     ① now_merchant_id（推荐）—— 直接填后台 merchant_id，与 C 端 publish 一致，优先于 POI。
//     ② location_poi_id —— 外部地图 POI id；不传 now_merchant_id 时，与商户 address_poi_id
//        字符串相等则自动挂商户。两者都传时 now_merchant_id 优先，POI 只写地址不落商户。
//     - 切勿用空字符串去匹配 POI（会误挂）；本脚本仅在非空时发送 location_poi_id。
...
type NowInput struct {
	...
	NowMerchantID   string `json:"now_merchant_id"`   // 可选，直接挂商户；优先于 location_poi_id

	// 媒体：http(s) 链接或本地路径，按顺序。第一张图片会被后端自动取为封面。
	Images []string `json:"images"`
	Videos []string `json:"videos"` // 视频（可选）；上传后 media_type=2

	// 位置（可选）。now_merchant_id 直挂优先；否则 location_poi_id 命中商户 address_poi_id 时自动关联。
...
func main() {
	...
	for i, in := range inputs {
		...
		if in.NowMerchantID != "" {
			payload["now_merchant_id"] = in.NowMerchantID
		}
		if in.LocationPoiID != "" {
