...
> 增量更新（IM 建群、`group_id`、脚本健壮性等）见 [CHANGELOG.md](CHANGELOG.md)。
...
- [`group/main.go`](group/main.go) —— **IM 自动建群**：调腾讯 IM 建群（含 SDKAppID/密钥），气泡可带 `group_id`（见第十五节）

> 一条龙顺序：**查重 → 建用户 →（可选 建群）→ 建商户 → 建气泡（可带 group_id）**。
> 前四个工具只依赖 Go 标准库；`group` 额外用腾讯官方签名库（项目已引入）。
...
> **不依赖后台 token**，只要有 IM 的 SDKAppID + 密钥即可。已封装进 [`group/main.go`](group/main.go)。
...
> 工具已内置以上默认；要换成你自己的用 `-sdkappid` / `-key`（或 env `BUZZ_IM_SDKAPPID` / `BUZZ_IM_KEY`）。
...
- **加群成员**：后端/本工具**不做服务端拉人**（无 add_group_member 封装），成员由客户端 IM SDK 自行加入；C 端用 `POST /api/v1/msg/group/join/report` 上报加入记录。

### 命令

```bash
# 单个建群（群主=发布者 user_id）
go run ./scripts/import/group create -owner <user_id> -name "示例气泡群" -type Public

# 批量建群（owner 不存在时可加 -import-owner 或在 JSON 里填 import_owner_nick）
go run ./scripts/import/group create -f scripts/import/group/sample.json -out groups.result.json

# 预注册 IM 账号（群主不存在时）
go run ./scripts/import/group import-account -uid <user_id> -nick 张三 -avatar https://x/a.jpg

# 生成某 uid 的 UserSig（调试/登录）
go run ./scripts/import/group usersig -uid <user_id>

# 查群信息
go run ./scripts/import/group info -group @TGS#xxxx
```

### ✅ 后端改动说明：导入接口已支持 `group_id`（建好的群可一步挂到气泡）

> **本次已改后端**（之前 `POST /internal/nows` 不接受 `group_id`，建好的群挂不上气泡）。改动如下：

| 位置 | 改动 |
|---|---|
| [`app/dto/admin_now.go`](../../app/dto/admin_now.go) | `AdminNowStoreReq` 与 `AdminNowUpdateReq` 都增 `group_id string`（更新时**空字符串=不改**）；响应 `AdminNowItem` 增 `group_id` |
| [`app/service/admin_now.go`](../../app/service/admin_now.go) | `Store` 写入 `entity.GroupID = in.GroupID`；`Update` 在 `in.GroupID != ""` 时 `data["group_id"]`；`toAdminNowItem` 回填 `group_id` |

所以现在 **`POST /internal/nows` 和 `PUT /internal/nows/:id` 都能直接带 `group_id`**，`now` 导入脚本也加了 `group_id` 字段 → 建群和建气泡可一趟打通，无需再写库。

### 🎯 完整示例：建用户 → 建群 → 建带群的气泡（爬虫可直接照抄）

```bash
# 0) 凭据（已内置在工具里，照抄即可；要换成自己的再覆盖）
export BUZZ_API_BASE="http://test-go-api.nowmap.cn"   # 后端地址
export BUZZ_TOKEN="粘贴后台登录拿到的token"             # 或用 -admin-user/-admin-pass 登录
# 腾讯 IM 凭据（建群用，已内置默认，无需 export，列出供参考）：
#   SDKAppID = 1600107795
#   IM Key   = 34b157159d5b5f21c5b6b02e43d3fb4e904b1a3c68092585e9cd36b67c841b9d
#   identifier = administrator

# 1) 建发布者用户（拿到 user_id）
go run ./scripts/import/user -token "$BUZZ_TOKEN" -f users.json
#   → users.result.json 里拿到 user_id，比如 U1001

# 2) 用该 user_id 当群主建 IM 群（拿到 group_id）
go run ./scripts/import/group create -owner U1001 -name "周末桌游局" -type Public
#   → ★ 建群成功 group_id=@TGS#aBcDeFg     （群主不存在就加 -import-owner）

# 3) 建气泡时直接带上 group_id（后端已支持，一步挂好）
cat > now1.json <<'JSON'
[{
  "user_id": "U1001",
  "now_title": "周末桌游，缺2人",
...
#   → ★ 气泡创建成功 now_id=...；用 query 核对 group_id 已写入：
go run ./scripts/import/query nows -token "$BUZZ_TOKEN" -keyword=周末桌游 | grep group_id
```

一条龙批量版（每条气泡配一个群）：**建用户 → 按发布者建群、把 `group_id` 回填进各自的气泡 JSON → 建气泡**。`group create -f` 的 `groups.result.json`（`owner → group_id`）就是回填用的台账。

> 备注：成员仍由客户端 IM SDK 自行加入（后端无服务端拉人接口）；建群默认只有群主在群里。
