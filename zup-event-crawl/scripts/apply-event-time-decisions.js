...
/**
 * 将 Agent 编写的 time-decisions.json 写入 review.db。
 *
 *   node scripts/apply-event-time-decisions.js --city=上海
 *   node scripts/apply-event-time-decisions.js --file=path/to/time-decisions.json
 */

const fs = require("fs");
const path = require("path");
const { applyEventTime, openDatabase } = require("../lib/review-db");
const { validateTimeDecision } = require("../lib/event-time-agent");
...
function main() {
  ...
  try {
    for (const decision of decisions) {
      ...
      const check = validateTimeDecision(decision);
      if (!check.ok) {
        summary.fail += 1;
      ...
      if (options.dryRun) {
        summary.ok += 1;
        console.log(`[dry-run] ${eventUid} → ${decision.start_at} ~ ${decision.expired_at}`);
        continue;
      }
      try {
