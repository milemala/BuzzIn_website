...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    else if (arg === "--skip-compose") options.skipCompose = true;
...
async function persistResults(options, listPath, report, merchantRows) {
  if (!options.dryRun && merchantRows.length) {
    ...
    if (!options.skipCompose && merchantRows.length) {
      console.log("\n合成 16:9 封面（模糊底图 + 原图居中）…");
      const composeReport = await batchComposeMerchantImages(db, {
        merchant_uids: merchantRows.map((row) => row.merchant_uid),
        rootDir: root,
        concurrency: 3,
      });
      console.log(`16:9 封面: ${composeReport.ok} 成功 / 跳过 ${composeReport.skip_done} / 无图 ${composeReport.skip_no_source} / 失败 ${composeReport.fail}`);
    }
